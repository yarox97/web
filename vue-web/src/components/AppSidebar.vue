...
.sidebar-inner {
  ...
  box-sizing: border-box; /* Важно, чтобы паддинги не увеличивали высоту */
}
...
.user-section {
  ...
  margin-bottom: 20px;
  padding: 10px;
  flex-shrink: 0; /* Не сжимать профиль */
}
...
.logout-section {
  ...
  flex-shrink: 0; /* Не сжимать кнопку выхода */
}
...
/* Small visual fix for the badge to handle larger numbers */
.badge {
  position: absolute;
  top: -5px;
  right: -8px;
  ...
  height: 16px;
  min-width: 16px;
  padding: 0 4px; /* Added padding for double digits like "99" */
  border-radius: 10px;
  display: flex;
  align-items: center;
  justify-content: center;
...
@media (max-width: 768px) {
  .close-btn {
    display: block; /* Появляется только на мобильном */
  }
}
</style>